...
namespace sardine
{
    ...
    void register_region_host(py::module_ m)
    {
        ...
        m.def("open", [](std::string name, py::object data_type) -> py_array {
        ...
        m.def("create", [](std::string name, std::vector<py::ssize_t> shape, py::object data_type) -> py_array {
            auto dtype = py::dtype::from_args(data_type);
        ...
        m.def("open_or_create", [](std::string name, std::vector<py::ssize_t> shape, py::object data_type) -> py_array {
            auto dtype = py::dtype::from_args(data_type);
            size_t bytes_size = dtype.itemsize(); for (auto e : shape) bytes_size *= e;
